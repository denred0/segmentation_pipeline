...
if __name__ == "__main__":
    ...
    output_dir = Path('data/perspective_transform/output')
    if output_dir.exists() and output_dir.is_dir():
        shutil.rmtree(output_dir)
    Path(output_dir).mkdir(parents=True, exist_ok=True)

    source_images = get_all_files_in_folder(Path("data/perspective_transform/input"), ["*.jpg"])
    ...
    for im in tqdm(source_images):
        ...
        # warped_show = cv2.resize(warped, (int(1920*0.8), int(1080*0.8)))
        # cv2.imshow("Warped", warped_show)
        # cv2.waitKey(0)
